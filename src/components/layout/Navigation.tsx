...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="mx-auto max-w-[1600px] flex items-center justify-between h-[72px] px-6 lg:px-10">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "relative flex items-center gap-2 px-3.5 sm:px-4 py-2.5 rounded-lg text-[13px] font-medium transition-colors min-h-[44px] flex-shrink-0",
                  isActive
                    ? "text-foreground"
